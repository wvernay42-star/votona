...
- **Sourcing des actus** : automatisé via la routine cloud « Votona - relance inactifs » (tous les jours à 4h07 UTC ≈ 6h à Paris, visible sur claude.ai/code/routines) qui cherche l'actualité majeure de la campagne et l'ajoute à la file `relance_news_queue` — elle n'envoie jamais d'email et ne touche à aucun fichier du dépôt. Elle lit la clé Supabase dans la variable d'environnement `SUPABASE_SERVICE_ROLE_KEY` de l'environnement cloud (jamais écrite dans un prompt ni dans ce dépôt). L'ancienne tâche locale Claude Desktop du même nom est abandonnée.
