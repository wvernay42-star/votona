...
- Sondages (bloc `POLL_DATA` d'`index.html`) : `CANDIDATE_POLL_PREV` / `CANDIDATE_POLL_PREV_AT` gardent le relevé PRÉCÉDENT et alimentent les flèches de tendance (places gagnées/perdues) du tableau de bord. À chaque mise à jour des sondages, recopier d'abord l'ancien `CANDIDATE_POLL_PCT` et l'ancienne `CANDIDATE_POLL_UPDATED_AT` dedans, puis seulement écrire les nouvelles valeurs.
- IndexNow (Bing & co) : `.github/workflows/indexnow.yml` signale automatiquement les pages modifiées (`index.html`, `candidats/`, `sujets/`, `comparer/`, `sitemap.xml`) après chaque push sur `main`, via `scripts/indexnow.py`. Le fichier `<clé>.txt` à la racine (32 caractères hexadécimaux) est la clé IndexNow : ne le supprime pas et ne le renomme pas. Lancement manuel possible depuis l'onglet Actions de GitHub pour signaler tout le sitemap.
...
- Mise en page : une seule largeur de contenu partout (880px, marges intérieures `clamp(20px,4vw,40px)`), dans l'app (`main`, barres fixes, pied de page) comme dans les pages générées (`main` + `--gutter`). Ne réintroduis pas de largeur propre à un écran ou à une page.
- Les pages `candidats/<id>/index.html` (une par candidat, plus `candidats/index.html`), les pages `sujets/<slug>/index.html` (une par sujet : qui est pour/contre, plus `sujets/index.html`), le comparateur `comparer/index.html` (deux candidats côte à côte, adresse partageable `/comparer/?a=<id>&b=<id>`) et `sitemap.xml` sont générés, pas édités à la main (voir `scripts/generate-candidate-pages.js`, qui réécrit le sitemap en entier : ne l'édite pas à la main). L'adresse d'un sujet est tirée de son intitulé : changer un intitulé change son adresse. Ce calcul existe en double (`slugify`/`topicSlugs` du générateur et `topicPageHref()` d'`index.html`, qui fait les liens de l'app vers les pages sujets) : si tu en modifies un, modifie l'autre à l'identique. Une fiche candidat sans aucune position connue affiche un message court, est en `noindex` et sort du sitemap ; elle y revient toute seule dès que la veille lui ajoute une position et que le script est relancé. Ce script demande Node.js : absent de la machine locale, il se lance depuis une session cloud. Après une modification des candidats/sujets, relance-le et commite les pages régénérées avec le reste (indispensable sur `draft`). Filet de sécurité sur `main` : l'action `.github/workflows/pages.yml` relance ce script et `scripts/generate-og-images.js` (images de partage manquantes, jamais réécrites : `candidats/<id>/og.jpg` et `assets/og/sujets/<slug>.jpg`, hors de `sujets/` que le générateur recrée ; celle d'un sujet renommé est refaite et l'ancienne supprimée) à chaque push qui touche `index.html`, commite le résultat s'il a changé (commit « Pages générées : mise à jour automatique… » de github-actions) puis prévient IndexNow. Elle ne modifie jamais `index.html`. D'où un `git pull` indispensable avant de travailler.
